...
- pipeline filter/dedupe/delivery/finalize 事件来自 `src/workflow/run_source/run_source_lifecycle.ts`、`src/workflow/run_source/run_source_execution_pipeline.ts` 与 `src/workflow/run_source/stages/delivery_stage.ts` / 相关 stages。
...
- `pretty` 展示 MAY 拍平高频业务键（如 `source.id`、`source.run_id`、`pipeline.item_id`、`delivery.id`、`http.request.method`、`http.route`、`http.response.status_code`、`template.ai.model_ref`），也 MAY 隐藏低频字段，但这些都 MUST 只发生在展示层。
