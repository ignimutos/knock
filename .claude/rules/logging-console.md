...
- source fetch/parse 事件来自 adapters source gateways（`src/adapters/sources/http_source_input_gateway.ts`、`src/adapters/sources/byparr_source_input_gateway.ts`、`src/adapters/sources/source_parser_gateway.ts`）。
- pipeline filter/dedupe/delivery/finalize 事件来自 `src/workflow/run_source/run_source_lifecycle.ts`、`src/workflow/run_source/run_source_execution_pipeline.ts` 与 `src/workflow/stages/delivery_stage.ts` / 相关 stages。
...
- `warn` / `error` / `fatal` 展示 SHOULD 优先呈现真实且相关的结果、原因与 trace 关联上下文；若不存在，就直接省略，而不是补占位字段。
- `debug` / `trace` 展示 MAY 暴露更多真实诊断属性，以支持排障；但仍 MUST 基于同一条底层 record，而不是拼接展示层私货。
- `info` 展示 SHOULD 以最小字段集为主，只在确有助于理解当前事件时再附带少量相关上下文。
- `pretty` 展示 MAY 拍平高频业务键（如 `source.id`、`source.run_id`、`pipeline.item_id`、`delivery.id`、`web.request_id`、`http.request.method`、`http.route`、`http.response.status_code`、`template.ai.model_ref`），也 MAY 隐藏低频字段，但这些都 MUST 只发生在展示层。
- `pretty` SHOULD 按级别着色；颜色只属于展示层，MUST NOT 反写到底层 JSON record。
