...
- `pretty` 展示 MAY 拍平高频业务键（如 `source.id`、`source.run_id`、`pipeline.item_id`、`delivery.id`、`web.request_id`、`http.request.method`、`http.route`、`http.response.status_code`），也 MAY 隐藏低频字段，但这些都 MUST 只发生在展示层。
