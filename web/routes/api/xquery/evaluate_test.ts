...
test('[flow] R19 xquery api: 应将请求 payload 原样转发给 evaluatePlayground', async () => {
...
test('[flow] R19 xquery api: POST 应返回 JSON 结果并上报成功日志元数据', async () => {
...
test('[flow] R20 xquery api: 非法 JSON 应返回 400 与 validation 错误体', async () => {
  const logs: EvaluateLogMeta[] = []
...
test('[contract] R19 xquery api: xquery 契约错误应返回 400 与结构化错误体', async () => {
  const response = await handler(
    new Request('http://localhost/api/xquery/evaluate', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
...
test('[flow] R19 xquery api: 抓取失败应返回 502 与结构化错误体并上报失败日志元数据', async () => {
  const logs: EvaluateLogMeta[] = []
...
test('[contract] xquery api: evaluation 失败应返回 422 与结构化错误体', async () => {
  const response = await handler(
    new Request('http://localhost/api/xquery/evaluate', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
...
test('[contract] xquery api: 未知错误应返回 500 与结构化错误体', async () => {
  const response = await handler(
    new Request('http://localhost/api/xquery/evaluate', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
