...
import type { DeliveryDraft, DeliveryKind } from '../../islands/config_workbench_state.ts'

export type { DeliveryKind } from '../../islands/config_workbench_state.ts'
...
function parseIntOrUndefined(value: string, fieldName: string): number | undefined {
...
function parseNumberArray(value: string, fieldName: string): number[] | undefined {
  const trimmed = value.trim()
  if (trimmed === '') return undefined
  ...
  return trimmed.split(',').map((item) => {
    ...
    if (!/^\d+$/.test(part)) {
      throw new Error(`${fieldName} 必须是逗号分隔的整数列表`)
    }
...
export function buildGlobalPayload(state: GlobalFormState): {
  ...
  return {
    ...
    loggingFileRotationMaxFiles: parseIntOrUndefined(
      state.loggingFileRotationMaxFiles,
...
export function buildDeliveryPayload(state: DeliveryFormState): {
  ...
  return {
    ...
    fileRotationBackups: parseIntOrUndefined(state.fileRotationBackups, 'file.rotation.backups'),
    ...
    pushRetryLimit: parseIntOrUndefined(state.pushRetryLimit, 'push.http.retry.limit'),
    pushRetryStatusCodes: parseNumberArray(
      state.pushRetryStatusCodes,
    ...
    emailSmtpPort: parseIntOrUndefined(state.emailSmtpPort, 'email.smtp.port'),
...
export function buildSourcePayload(
  ...
  for (const deliveryId of state.deliveryIds) {
    ...
    if (!kind) {
      throw new Error(`未知 deliveryId: ${deliveryId}`)
    }
