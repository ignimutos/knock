...
test('[contract] createRunSourceUseCaseForRuntime: daemon production wiring 有完整 pipeline 时不应退化为 collect-only', async () => {
  let insertedRuns = 0
...
test('[contract] createSourceExecutionCore: 应复用 shared deps 并暴露统一 fetch/parse/render 入口', async () => {
  const lines: string[] = []
