...
} from '../../config/update_config_document.ts'
import {
...
function assertManagedFilesystemPaths(global: {
...
function assertManagedDeliveryPaths(
...
function buildStructuredDeliveryConfig(
  ...
  switch (request.kind) {
    ...
    case 'email': {
      ...
      next.message = pruneUndefined(message)
      return pruneUndefined(next)
    }
  }
}

export async function updateGlobalConfig(input: unknown): Promise<{
  message: string
...
export async function upsertDeliveryConfig(input: unknown): Promise<{
  message: string
  workbench: ConfigWorkbenchOverview
}> {
  let request
  ...
  assertManagedDeliveryPaths(config, request.kind)
...
export async function deleteDeliveryConfig(input: unknown): Promise<{
  message: string
  workbench: ConfigWorkbenchOverview
}> {
  let request
