...
Deno.test(
  '[contract] source management: updateSourceConfig 应写回 source 子树并保留 keyed deliveries',
  async () => {
...
Deno.test(
  '[contract] source management: clearSourceHistory 应只删除指定 source 的 production facts',
  async () => {
    await withRuntimeDir(async (runtimeDir) => {
