...
test('[unit] domain: finalizeSourceRun 应按 attempt 汇总 success/partial/failed', () => {
...
test('[unit] domain: preview 与 production effectDomain 必须显式区分', () => {
...
test('[unit] domain: finalizeSourceRun 应将无投递且有跳过事实的 run 归类为 skipped 并写入 finishedAt', () => {
  const run = createSourceRun({
    runId: 'run-skipped',
    sourceId: 'rust',
    trigger: 'scheduled',
...
test('[unit] domain: createSourceRun 应拒绝 preview profile 与 production effectDomain 的非法组合', () => {
  assertThrows(() =>
    createSourceRun({
...
test('[unit] domain: createRunPlan 应拒绝非 preview profile 搭配 preview trigger', () => {
  assertThrows(() =>
    createRunPlan({
...
test('[unit] domain: createRunPlan 应拒绝 binding.deliveryId 与 definition.deliveryId 漂移', () => {
  assertThrows(() =>
    createRunPlan({
...
test('[unit] domain: delivery attempt 终态必须与 finishedAt 基本一致', () => {
  assertThrows(() =>
...
test('[unit] domain: delivery attempt 非终态不得携带 finishedAt', () => {
  assertThrows(() =>
    assertDeliveryAttemptInvariant({
...
test('[unit] domain: failed attempt 即使伴随 skipped/filter 事实也应归类为 failed', () => {
  const run = createSourceRun({
...
test('[unit] domain: 多个 delivery attempt 成功不应受 parsedCount 限制误伤', () => {
  const run = createSourceRun({
...
test('[unit] domain: delivery attemptNumber 必须是整数', () => {
  assertThrows(() =>
    assertDeliveryAttemptInvariant({
...
test('[unit] domain: renderedSnapshot.channel 必须与 attempt.channel 一致', () => {
  assertThrows(() =>
    assertDeliveryAttemptInvariant({
