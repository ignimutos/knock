...
import {
  createHttpClient,
  type ProxyClient,
  type ProxyClientFactoryOptions,
} from '../../core/http_client.ts'
...
function getRequestClient(init: RequestInit | undefined): ProxyClient | undefined {
  return (init as (RequestInit & { client?: ProxyClient }) | undefined)?.client
}
...
test('[contract] httpDelivery: body 请求应发送 JSON body 与合并后的 headers', async () => {
  const calls: Array<{
...
test('[contract] httpDelivery: query 请求应把 payload 编码到 query string', async () => {
...
test('[contract] httpDelivery: form 请求应默认设置 form content-type', async () => {
...
test('[contract] httpDelivery: query payload 非对象/字符串时应报错', async () => {
  const delivery = createHttpDelivery({
    httpClient: createHttpClient({
...
test('[contract] httpDelivery: form payload 非对象时应报错', async () => {
  const delivery = createHttpDelivery({
    httpClient: createHttpClient({
      fetcher: () => Promise.resolve(new Response('ok', { status: 200 })),
    }),
...
test('[contract] httpDelivery: 配置 http proxy 时应把 client 注入 fetch init 并在完成后关闭', async () => {
  const calls: Array<RequestInit | undefined> = []
  const createHttpClientCalls: Array<ProxyClientFactoryOptions> = []
  let closeCalls = 0
  const proxyClient = {
    close: () => {
      closeCalls += 1
...
test('[contract] httpDelivery: 配置 socks5 proxy 时应把 client 注入 fetch init 并在完成后关闭', async () => {
  const calls: Array<RequestInit | undefined> = []
  const createHttpClientCalls: Array<ProxyClientFactoryOptions> = []
  let closeCalls = 0
  const proxyClient = {
...
test('[contract] httpDelivery: 成功响应时不应渲染 failure message 模板', async () => {
...
test('[contract] httpDelivery: transport throw 时应记录统一 failure 日志', async () => {
  const logs: string[] = []
  const logger = createLogger({
    enabled: true,
    level: 'info',
...
test('[contract] httpDelivery: 2xx 且无 response 检查时 invalid JSON 不应导致失败', async () => {
  const delivery = createHttpDelivery({
    httpClient: createHttpClient({
      fetcher: () =>
        Promise.resolve(
...
test('[contract] httpDelivery: 需要 response body 时 invalid JSON 应记录 parse failure 日志', async () => {
  const logs: string[] = []
  const logger = createLogger({
    enabled: true,
    level: 'info',
...
test('[contract] httpDelivery: predicate render throw 时应记录 predicate render failure 日志', async () => {
  const logs: string[] = []
  const logger = createLogger({
    enabled: true,
    level: 'info',
...
test('[contract] httpDelivery: message render throw 时应记录 message render failure 日志', async () => {
  const logs: string[] = []
  const logger = createLogger({
    enabled: true,
    level: 'info',
...
test('[flow] R07 httpDelivery: 非 2xx 响应时应抛错并记录 failure 日志', async () => {
  const logs: string[] = []
  let closeCalls = 0
  const proxyClient = {
    close: () => {
