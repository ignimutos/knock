...
test('[contract] byparrSourceInputGateway: 抓取成功应记录 payload-free source 日志', async () => {
  const logs: string[] = []
...
test('[contract] byparrSourceInputGateway: 抓取失败应记录失败日志且不泄漏 payload', async () => {
  const logs: string[] = []
...
test('[contract] byparrSourceInputGateway: 200 非法 JSON 失败日志必须使用稳定安全错误消息', async () => {
  const logs: string[] = []
