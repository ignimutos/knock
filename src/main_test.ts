...
Deno.test('parseCliArgs: 应解析 --config、--runtime_dir 与 --immediate', () => {
...
Deno.test('parseCliArgs: 返回值应可赋给 app 启动入口类型', () => {
...
Deno.test('toDaemonStartOptions: 应收敛为 daemon 启动参数', () => {
...
Deno.test('toDaemonStartOptions: 返回值应可赋给 app 启动入口类型', () => {
...
Deno.test('resolveDaemonStartOptions: CLI 显式 runtime_dir 应优先于环境变量', () => {
...
Deno.test('resolveDaemonStartOptions: 未传 runtime_dir 时应回退到环境变量', () => {
  const options = resolveDaemonStartOptions(
...
Deno.test('parseCliArgs: 应解析 mode=web 与 web 参数', () => {
...
Deno.test('parseCliArgs: start 默认 mode=all', () => {
  assertEquals(parseCliArgs([]).mode, 'all')
})

Deno.test('parseCliArgs: web 模式不接受 --config', () => {
  assertThrows(
...
Deno.test('parseCliArgs: web 模式不接受 --runtime_dir', () => {
  assertThrows(
...
Deno.test('parseCliArgs: web 模式不接受 --immediate', () => {
  assertThrows(
...
Deno.test('parseCliArgs: daemon 模式不接受 --web_host', () => {
  assertThrows(
...
Deno.test('parseCliArgs: daemon 模式不接受 --web_port', () => {
  assertThrows(
...
Deno.test('parseCliArgs: --web_port 空白时应报错', () => {
  assertThrows(() => parseCliArgs(['--mode', 'web', '--web_port', '  ']), Error, '--web_port 非法')
})

Deno.test('parseCliArgs: --web_port 越界时应报错', () => {
  assertThrows(
...
Deno.test('parseCliArgs: web 模式下 --config 空字符串也应报互斥错误', () => {
  assertThrows(
...
Deno.test('buildChildArgs: all 模式参数可分发到 daemon 子进程', () => {
...
Deno.test('buildChildArgs: all 模式参数可分发到 web 子进程', () => {
  const parsed = parseCliArgs([
    '--config',
    'runtime/config.yml',
    '--web_host',
