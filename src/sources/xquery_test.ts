...
Deno.test('xquery: 原生表达式映射应产出 entry.id', () => {
...
Deno.test('xquery: 原生表达式支持 HTML 提取', () => {
  const html =
...
Deno.test('xquery: 大写 DOCTYPE HTML 也应按 HTML 处理', () => {
  const html =
...
Deno.test('xquery: HTML 前置注释时也应按 HTML 处理', () => {
  const html =
...
Deno.test('xquery: HTML 文档中的属性与文本提取应保持现有语义', () => {
  const html =
...
Deno.test('xquery: XHTML 文档应按 XML 处理并保留 namespace 语义', () => {
...
Deno.test('xquery: 原生表达式可直接生成常量字段', () => {
...
Deno.test('xquery: template 前缀不再受支持', () => {
  const xml = `<root><ul><li data-id="1"><a href="/a">Hello</a></li></ul></root>`
...
Deno.test('xquery: literal 前缀不再受支持', () => {
  const xml = `<root><ul><li data-id="1"><a href="/a">Hello</a></li></ul></root>`
...
Deno.test('xquery: xquery 前缀不再受支持', () => {
  const xml = `<root><ul><li data-id="1"><a href="/a">Hello</a></li></ul></root>`
...
Deno.test('xquery: locate 缺省时应以 document 作为 entry 上下文执行一次', () => {
...
Deno.test('xquery: feed 支持脚本字符串返回 map', () => {
...
Deno.test('xquery: entry 支持脚本字符串并按 locate 节点执行', () => {
...
Deno.test('xquery: entry 脚本返回缺少 id 时应报错', () => {
...
Deno.test('xquery: 脚本模式不使用 namespaces', () => {
  const xhtml = `<?xml version="1.0"?>
<html xmlns="http://www.w3.org/1999/xhtml">
  <body>
    <ul>
