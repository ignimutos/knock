import { loadConfigRuntimeContext, type ConfigRuntimeContext } from './runtime_config_context.ts'
import type { RawConfigDocumentLoadResult } from './raw_config_document.ts'
import type { ConfigWorkbenchOverview, ReaderOverview } from '../contracts/workbench.ts'
import { normalizeWorkbenchIssue } from './workbench_overview.ts'
...
export async function buildReaderOverviewFromSession(
  ...
  const { buildCurrentReaderOverview } = await import('./reader_overview.ts')
...
export async function buildWorkbenchOverviewFromSession(
  ...
  const { buildConfigWorkbenchOverview } = await import('./workbench_overview.ts')
