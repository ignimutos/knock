...
Deno.test(
  "[contract] docker/deno.compile.json: 应与 deno.json 保持运行时依赖同步并排除 build-time imports",
  () => {
    ...
    const expectedImports = Object.fromEntries(
      Object.entries(baseConfig.imports ?? {}).filter(([key]) =>
        !excludedImports.has(key)
      ),
    );

    assertEquals(baseConfig.imports?.vite, "npm:vite@^7.3.2");
    assertEquals(
      baseConfig.imports?.["@fresh/plugin-vite"],
