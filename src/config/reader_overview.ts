...
import { redactConfigSecrets } from '../web/config_secret_redaction.ts'
import { getRawSourceDeliveryOverrides } from './source_delivery_overrides.ts'
import { createDbClient, type FactsDbClient } from '../persistence/sqlite/client.ts'
import { parseRawConfigDocument, type LoadedCompiledConfig } from './load_compiled_config.ts'
import { readTextFile } from '../platform/fs.ts'
import { PIPELINE_ITEM_STATUSES, SOURCE_RUN_STATUSES } from '../persistence/sqlite/schema.ts'
...
export async function buildReaderOverview(input: {
  ...
  const sources = await Promise.all(
    input.config.sources.map(async (source) => {
      const lastRunRow = await readModel.getLatestRunForSource(source.id, 'production')
      const lastRun = lastRunRow ? toLastRun(lastRunRow) : undefined
      const entryRows = lastRun
        ? (await readModel.getEntriesForRun(lastRun.runId, 'production')).map(toEntryRow)
        : []
