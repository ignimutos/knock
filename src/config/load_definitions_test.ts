...
test('[contract] loadDefinitions: 应将 resolved config 组装成判别联合 SourceDefinition 与 DeliveryBinding', async () => {
  ...
  const archiveBinding = rustArchiveBinding!
  const mailerBinding = rustMailerBinding!
  assertEquals('profile' in archiveBinding, false)
  assertEquals('effectDomain' in archiveBinding, false)
  assertEquals('trigger' in archiveBinding, false)
  if (fileDelivery?.kind !== 'file' || archiveBinding.definition.kind !== 'file') {
    throw new Error('archive delivery 应为 file')
  }
  assertEquals(fileDelivery.contentTemplate, '{{ entry.title }}')
  assertEquals(archiveBinding.definition.contentTemplate, 'override {{ entry.id }}')
  assertNotEquals(archiveBinding.definition, fileDelivery)

  if (emailDelivery?.kind !== 'email' || mailerBinding.definition.kind !== 'email') {
    throw new Error('mailer delivery 应为 email')
  }
  assertEquals(emailDelivery.messageTemplate.subject, '[{{ source.title }}] {{ entry.title }}')
  assertEquals(mailerBinding.definition.messageTemplate.subject, '[override] {{ entry.title }}')
})
