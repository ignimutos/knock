...
test('[contract] validateConfig: source.deliveries 数组输入应按 keyed override 契约拒绝', () => {
  const input = {
    runtimeDir: '/tmp/runtime',
    deliveries: {
      local: {
  ...
  assertStringIncludes(err.message, 'source.feed.deliveries')
...
test('[contract] validateConfig: delivery 根节点出现非 canonical target 字段时应拒绝', () => {
  const input = {
    runtimeDir: '/tmp/runtime',
    deliveries: {
      webhook: {
  ...
  const err = assertThrows(() => validateConfig(input), Error)
  assertStringIncludes(err.message, 'delivery.webhook.http')
  assertStringIncludes(err.message, '非法')
})

test('[contract] validateConfig: push.request 之外声明 payload type 字段时应拒绝', () => {
  const input = {
    runtimeDir: '/tmp/runtime',
    deliveries: {
      webhook: {
