...
Deno.test('sqlite v2: query service 应回读真实 skippedReason', async () => {
  const db = createInMemoryDb()
...
Deno.test('sqlite v2: query service 遇到坏 counts_json 应失败而不是伪装成合法 run', async () => {
  const db = createInMemoryDb()
...
Deno.test('sqlite v2: query service 遇到跨字段非法 source run 行应失败', async () => {
  const db = createInMemoryDb()
...
Deno.test(
  'sqlite v2: query service 遇到坏 normalized_json 应失败而不是伪装成合法 item',
  async () => {
    const db = createInMemoryDb()
...
Deno.test(
  'sqlite v2: query service 遇到坏 rendered_snapshot_json 应失败而不是伪装成合法 attempt',
  async () => {
    const db = createInMemoryDb()
