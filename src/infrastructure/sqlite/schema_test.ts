...
Deno.test(
  'sqlite v2: schema 应初始化 source_runs pipeline_items delivery_attempts deduplications 表',
  () => {
...
Deno.test('sqlite v2: deduplication repository 应按 effectDomain + scope 判定重复', async () => {
  const db = createInMemoryDb()
...
Deno.test('sqlite v2: schema 应将 effect_domain 纳入 run/item/attempt 跨记录外键', () => {
  const db = createInMemoryDb()
...
Deno.test(
  'sqlite v2: pipeline item 应要求 source_run_id 指向已存在 source_runs.run_id',
  async () => {
    const db = createInMemoryDb()
...
Deno.test('sqlite v2: delivery attempt 应要求 item_id 指向已存在 pipeline item', async () => {
  const db = createInMemoryDb()
...
Deno.test('sqlite v2: delivery attempt 不应允许 source_run_id 脱离所属 pipeline item', async () => {
  const db = createInMemoryDb()
...
Deno.test('sqlite v2: pipeline item 不应允许引用不同 effect_domain 的 run', async () => {
  const db = createInMemoryDb()
...
Deno.test('sqlite v2: delivery attempt 不应允许引用不同 effect_domain 的 run', async () => {
  const db = createInMemoryDb()
...
Deno.test('sqlite v2: delivery attempt 不应允许引用不同 effect_domain 的 item', async () => {
  const db = createInMemoryDb()
