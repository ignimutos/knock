...
Deno.test('[contract] xquery_playground: mapping 模式请求应转换为 xquerySchema 形状', () => {
...
Deno.test('[contract] xquery_playground: byparr 模式请求应转换为 byparr source', () => {
  const parsed = parsePlaygroundRequest({
    runtime: 'byparr',
    url: 'https://example.com/page.html',
...
Deno.test('[contract] xquery_playground: script 模式 + namespaces 应产生 warning', () => {
  const parsed = parsePlaygroundRequest({
    url: 'https://example.com/page.html',
    namespaces: { xh: 'http://www.w3.org/1999/xhtml' },
...
Deno.test('[contract] xquery_playground: 缺少 entry.id 时应沿用既有错误契约', () => {
  assertThrows(
    () =>
      parsePlaygroundRequest({
        url: 'https://example.com/page.html',
...
Deno.test('[contract] xquery_playground: 应拒绝私网 IPv4 地址', () => {
  assertThrows(
    () =>
      parsePlaygroundRequest({
...
Deno.test('[contract] xquery_playground: 应拒绝非 http 协议', () => {
  assertThrows(
    () =>
      parsePlaygroundRequest({
        url: 'file:///etc/passwd',
...
Deno.test(
  '[contract] xquery_playground: classifyPlaygroundError 应将 __required__ 映射为 validation',
  () => {
...
Deno.test(
  '[contract] xquery_playground: classifyPlaygroundError 应将内网限制映射为 validation',
  () => {
...
Deno.test(
  '[contract] xquery_playground: classifyPlaygroundError 应将 __illegal__ 映射为 validation',
  () => {
...
Deno.test('[contract] xquery_playground: classifyPlaygroundError 应保留带路径的 非法 文案', () => {
...
Deno.test(
  '[contract] xquery_playground: classifyPlaygroundError 应将抓取失败映射为 fetch 并清洗文案',
  () => {
...
Deno.test(
  '[contract] xquery_playground: classifyPlaygroundError 应将 xquery 契约错误映射为 evaluation',
  () => {
...
Deno.test(
  '[contract] xquery_playground: classifyPlaygroundError 应将普通 Error 归为 internal',
  () => {
...
Deno.test('[contract] xquery_playground: classifyPlaygroundError 应对非 Error 回退运行失败', () => {
...
Deno.test(
  '[contract] xquery_playground: evaluatePlayground 遇到非 2xx 响应时应保留底层抓取错误',
  async () => {
    await assertRejects(
      () =>
        evaluatePlayground({
...
Deno.test(
  '[contract] xquery_playground: evaluatePlayground 遇到 xquery 执行失败时应保留原始错误',
  async () => {
    await assertRejects(
      () =>
        evaluatePlayground({
