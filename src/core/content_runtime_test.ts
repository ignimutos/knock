...
test('[contract] contentRuntime: buildContext 应默认将 entry 拍平到模板顶层并保留命名空间', () => {
...
test('[contract] contentRuntime: shouldPassFilter 支持顶层 entry 字段', async () => {
...
test('[contract] contentRuntime: shouldPassFilter 顶层 entry 字段返回 false 时拦截', async () => {
  const passed = await shouldPassFilter('{{ title == "Rust" }}', {
    title: 'Go',
    entry: { title: 'Go' },
    feed: { title: 'Feed' },
...
test('[contract] contentRuntime: shouldPassFilter 顶层 entry 字段支持 match_fuzzy', async () => {
...
test('[contract] contentRuntime: shouldPassFilter 顶层 entry 字段支持 match_regex', async () => {
...
test('[contract] contentRuntime: shouldPassFilter 支持反转 match_regex', async () => {
...
test('[contract] contentRuntime: shouldPassFilter 顶层 entry 字段非法 regex 会透传错误', async () => {
  await assertRejects(
    () =>
...
test('[contract] contentRuntime: shouldPassFilter 仍支持 entry 命名空间写法', async () => {
...
test('[contract] contentRuntime: shouldPassFilter 支持 extract_regex 提取后比较', async () => {
...
test('[contract] contentRuntime: shouldPassFilter 提取失败时可继续参与数值比较', async () => {
  const passed = await shouldPassFilter(
...
test('[contract] contentRuntime: shouldPassFilter 渲染失败应报错并中止', async () => {
  await assertRejects(() => shouldPassFilter('{{', { entry: { title: 'Rust' } }), Error)
})

test('[contract] contentRuntime: renderContent 应支持顶层 entry 字段渲染字符串模板', async () => {
...
test('[contract] contentRuntime: renderContent 仍支持 entry 命名空间', async () => {
...
test('[contract] contentRuntime: renderContent 顶层 entry 字段应继续支持共享 runtime filter', async () => {
...
test('[contract] contentRuntime: renderContent 纯文本 content 应按字面量渲染', async () => {
  const out = await renderContent('hello', { entry: { title: 'Hello' } })
  assertEquals(out, 'hello')
})

test('[contract] contentRuntime: renderContent 应拒绝非法模板内容类型', async () => {
  await assertRejects(
    () =>
...
test('[contract] contentRuntime: renderPayload 应递归渲染 HTTP payload 中的 Liquid 字符串', async () => {
...
test('[contract] contentRuntime: 业务 runtime 在 to_telegram_html 正常路径输出 debug 日志', async () => {
...
test('[contract] contentRuntime: 业务 runtime 在 to_telegram_html 自动修正时输出 info 日志', async () => {
  const logs: Array<Record<string, unknown>> = []
  const logger = createLogger({
    enabled: true,
    level: 'debug',
...
test('[contract] contentRuntime: 业务 runtime 在 multiline 相对链接自动修正时仍只输出一条 info 日志', async () => {
  const logs: Array<Record<string, unknown>> = []
  const logger = createLogger({
    enabled: true,
    level: 'debug',
...
test('[contract] contentRuntime: shared runtime 的 to_telegram_html 不产生日志', async () => {
