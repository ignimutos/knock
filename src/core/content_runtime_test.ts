...
Deno.test(
  '[contract] contentRuntime: buildContext 应默认将 entry 拍平到模板顶层并保留命名空间',
  () => {
...
Deno.test('[contract] contentRuntime: shouldPassFilter 支持顶层 entry 字段', async () => {
...
Deno.test(
  '[contract] contentRuntime: shouldPassFilter 顶层 entry 字段返回 false 时拦截',
  async () => {
    const passed = await shouldPassFilter('{{ title == "Rust" }}', {
      title: 'Go',
      entry: { title: 'Go' },
...
Deno.test(
  '[contract] contentRuntime: shouldPassFilter 顶层 entry 字段支持 match_fuzzy',
  async () => {
...
Deno.test(
  '[contract] contentRuntime: shouldPassFilter 顶层 entry 字段支持 match_regex',
  async () => {
...
Deno.test('[contract] contentRuntime: shouldPassFilter 支持反转 match_regex', async () => {
...
Deno.test(
  '[contract] contentRuntime: shouldPassFilter 顶层 entry 字段非法 regex 会透传错误',
  async () => {
    await assertRejects(
      () =>
...
Deno.test('[contract] contentRuntime: shouldPassFilter 仍支持 entry 命名空间写法', async () => {
...
Deno.test(
  '[contract] contentRuntime: renderContent 应支持顶层 entry 字段渲染字符串模板',
  async () => {
...
Deno.test('[contract] contentRuntime: renderContent 仍支持 entry 命名空间', async () => {
...
Deno.test(
  '[contract] contentRuntime: renderContent 顶层 entry 字段应继续支持共享 runtime filter',
  async () => {
...
Deno.test('[contract] contentRuntime: renderContent 纯文本 content 应按字面量渲染', async () => {
  const out = await renderContent('hello', { entry: { title: 'Hello' } })
  assertEquals(out, 'hello')
})

Deno.test('[contract] contentRuntime: renderContent 应拒绝非法模板内容类型', async () => {
  await assertRejects(
    () =>
...
Deno.test(
  '[contract] contentRuntime: renderPayload 应递归渲染 HTTP payload 中的 Liquid 字符串',
  async () => {
...
Deno.test(
  '[contract] contentRuntime: 业务 runtime 在 to_telegram_html 正常路径输出 debug 日志',
  async () => {
...
Deno.test(
  '[contract] contentRuntime: 业务 runtime 在 to_telegram_html 自动修正时输出 info 日志',
  async () => {
    const logs: Array<Record<string, unknown>> = []
    const logger = createLogger({
      enabled: true,
...
Deno.test(
  '[contract] contentRuntime: 业务 runtime 在 multiline 相对链接自动修正时仍只输出一条 info 日志',
  async () => {
    const logs: Array<Record<string, unknown>> = []
    const logger = createLogger({
      enabled: true,
...
Deno.test('[contract] contentRuntime: shared runtime 的 to_telegram_html 不产生日志', async () => {
