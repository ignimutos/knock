...
test('[contract] R11 logger: runtime pretty 应输出高密度单行并隐藏块状字段', async () => {
  const stdout: string[] = []
  ...
  assertStringIncludes(plain, '2026-03-24 21:45:12')
  assertStringIncludes(plain, 'info')
  assertStringIncludes(plain, 'filter')
  assertStringIncludes(plain, 'component=daemon')
  assertStringIncludes(plain, 'source.id=smzdm')
  assertEquals(plain.includes('resource:'), false)
  assertEquals(plain.includes('attributes:'), false)
})
...
test('[contract] R11 logger: level=warn 时过滤 info 但保留 warn/error/fatal', () => {
  const stdout: string[] = []
  const stderr: string[] = []
...
test('[contract] R11 logger: 默认输出为严格 OTel JSON 并包含基础字段', () => {
  const stdout: string[] = []
...
test('[contract] R11 logger: 应优先从调用栈补全 code.* 属性且 run_id 不冒充 trace_id', () => {
  const stdout: string[] = []
...
test('[contract] R11 logger: ext 内部 runtime frame 应降级而不是误记为业务调用点', () => {
  const stdout: string[] = []
...
test('[contract] R11 logger: 有界缓存 helper 达到上限时应淘汰最旧 key', () => {
...
test('[contract] R11 logger: 栈解析失败时不中断日志输出', () => {
  const stdout: string[] = []
...
test('[contract] R11 logger: format=pretty 时应输出紧凑单行并保持脱敏', () => {
  const stdout: string[] = []
  ...
  const first = stripAnsi(stdout[0])
  assertStringIncludes(first, '2026-03-25 05:45:12')
  assertStringIncludes(first, 'http')
  assertStringIncludes(first, '推送完成')
  assertEquals(first.includes('123456:ABCDEF-SECRET'), false)
  assertEquals(first.includes('user:pass@'), false)
  assertEquals(first.includes('attributes:'), false)
  assertStringIncludes(stdout[1], '\u001b[')
  const second = stripAnsi(stdout[1])
  assertStringIncludes(second, '2026-03-25 05:45:12')
  assertStringIncludes(second, 'http')
  assertStringIncludes(second, '再次推送完成')
  assertEquals(second.includes('123456:ABCDEF-SECRET'), false)
  assertEquals(second.includes('user:pass@'), false)
})

test('[contract] R11 logger: pretty info 应隐藏低价值调试字段，只保留最小字段集与关键信息', () => {
  const stdout: string[] = []
  ...
  assertEquals(stdout.length, 1)
  assertStringIncludes(stdout[0], '\u001b[')
  const plain = stripAnsi(stdout[0])
  assertStringIncludes(plain, '2026-03-24 21:45:12')
  assertStringIncludes(plain, 'api')
  assertStringIncludes(plain, 'API 请求完成')
  assertStringIncludes(plain, 'web.req.1')
  assertStringIncludes(plain, '/api/xquery/evaluate')
  assertEquals(plain.includes('code.filepath'), false)
  assertEquals(plain.includes('pipeline.warning_count'), false)
  assertEquals(plain.includes('attributes:'), false)
})

test('[contract] R11 logger: pretty debug 应保留真实诊断字段', () => {
  const stdout: string[] = []
  const warn: string[] = []
  const stderr: string[] = []
  ...
  const debugPlain = stripAnsi(stdout[0])
  const warnPlain = stripAnsi(warn[0])
  const errorPlain = stripAnsi(stderr[0])
  assertStringIncludes(debugPlain, 'debug')
  assertStringIncludes(warnPlain, 'warn')
  assertStringIncludes(errorPlain, 'error')
  assertStringIncludes(debugPlain, 'pipeline.warning_count')
  assertStringIncludes(debugPlain, '/src/core/logger_test.ts')
})

test('[contract] R11 logger: module 只有一个事实源并写入 scope.name，业务字段进入 attributes', () => {
  const stdout: string[] = []
...
test('[contract] R11 logger: child 应保留 resource 并把 HTTP 语义字段写入标准 attributes', () => {
  const stdout: string[] = []
...
test('[contract] R11 logger: null 字段应直接省略而不是序列化为空字符串', () => {
  const stdout: string[] = []
  const logger = createLogger({
    enabled: true,
    level: 'info',
...
test('[contract] R11 logger: nested object 内部的 null 与 undefined 字段应被省略', () => {
  const stdout: string[] = []
  const logger = createLogger({
    enabled: true,
    level: 'info',
...
test('[contract] R11 logger: array 元素应保留旧标量语义', () => {
  const stdout: string[] = []
  const logger = createLogger({
    enabled: true,
    level: 'info',
...
test('[contract] R11 logger: 敏感字段与敏感内容应在 OTel attributes 中继续脱敏', () => {
  const stderr: string[] = []
...
export const testMeta = [
  {
    title: '[contract] R11 logger: console format=jsonl 应输出仓库 OTel JSONL',
    layer: 'contract',
    risks: ['R11'],
  },
  {
    title: '[contract] R11 logger: runtime pretty 应输出高密度单行并隐藏块状字段',
    layer: 'contract',
    risks: ['R11'],
  },
  {
    title: '[contract] R11 logger: enabled=false 时不输出日志',
    layer: 'contract',
    risks: ['R11'],
  },
  {
    title: '[contract] R11 logger: level=warn 时过滤 info 但保留 warn/error/fatal',
    layer: 'contract',
    risks: ['R11'],
  },
  {
    title: '[contract] R11 logger: 默认输出为严格 OTel JSON 并包含基础字段',
    layer: 'contract',
    risks: ['R11'],
  },
  {
    title: '[contract] R11 logger: 应优先从调用栈补全 code.* 属性且 run_id 不冒充 trace_id',
    layer: 'contract',
    risks: ['R11'],
  },
  {
    title: '[contract] R11 logger: 无函数名栈帧时仍输出 code.filepath 与 code.line.number',
    layer: 'contract',
    risks: ['R11'],
  },
  {
  ...
  {
    title: '[contract] R11 logger: ext 内部 runtime frame 应降级而不是误记为业务调用点',
    layer: 'contract',
    risks: ['R11'],
  },
  {
    title: '[contract] R11 logger: 归一化后的 self-frame 应被过滤并继续查找下一条业务帧',
    layer: 'contract',
    risks: ['R11'],
  },
  {
    title: '[contract] R11 logger: 缓存不会把上一条调用点误复用到下一条不同 stack line',
    layer: 'contract',
    risks: ['R11'],
  },
  {
    title: '[contract] R11 logger: 有界缓存 helper 达到上限时应淘汰最旧 key',
    layer: 'contract',
    risks: ['R11'],
  },
  {
    title: '[contract] R11 logger: Windows 风格分隔符归一化后 self-frame 仍会被过滤',
    layer: 'contract',
    risks: ['R11'],
  },
  {
    title: '[contract] R11 logger: 栈解析失败时不中断日志输出',
    layer: 'contract',
    risks: ['R11'],
  },
  {
    title: '[contract] R11 logger: format=pretty 时应输出紧凑单行并保持脱敏',
    layer: 'contract',
    risks: ['R11'],
  },
  {
    title: '[contract] R11 logger: pretty info 应隐藏低价值调试字段，只保留最小字段集与关键信息',
    layer: 'contract',
    risks: ['R11'],
  },
  {
    title: '[contract] R11 logger: pretty debug 应保留真实诊断字段',
    layer: 'contract',
    risks: ['R11'],
  },
  {
    title: '[contract] R11 logger: module 只有一个事实源并写入 scope.name，业务字段进入 attributes',
    layer: 'contract',
    risks: ['R11'],
  },
  {
    title: '[contract] R11 logger: child 应保留 resource 并把 HTTP 语义字段写入标准 attributes',
    layer: 'contract',
    risks: ['R11'],
  },
  {
    title: '[contract] R11 logger: null 字段应直接省略而不是序列化为空字符串',
    layer: 'contract',
    risks: ['R11'],
  },
  {
    title: '[contract] R11 logger: nested object 内部的 null 与 undefined 字段应被省略',
    layer: 'contract',
    risks: ['R11'],
  },
  {
    title: '[contract] R11 logger: array 元素应保留旧标量语义',
    layer: 'contract',
    risks: ['R11'],
  },
  {
    title: '[contract] R11 logger: 敏感字段与敏感内容应在 OTel attributes 中继续脱敏',
    layer: 'contract',
    risks: ['R11'],
  },
] as const
