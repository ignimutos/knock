...
Deno.test('logger: 默认输出为严格 OTel JSON 并包含基础字段', () => {
  ...
  assertEquals(attributes.run_id, 'source.rust.20260324T214512345Z')
  assertEquals(attributes.source_id, 'rust')
...
Deno.test('logger: 应优先从调用栈补全 code.* 属性且 run_id 不冒充 traceId', () => {
  const stdout: string[] = []
  ...
  const logger = createLogger({
    ...
    now: () => new Date('2026-03-24T21:45:12.345Z'),
    writeStdout: (line: string) => stdout.push(line),
  }).child({ runId: 'source.rust.20260324T214512345Z' })
...
Deno.test('logger: 敏感字段与敏感内容应在 OTel attributes 中继续脱敏', () => {
  ...
  assertEquals(attributes.delivery_id, 'rust__webhook__0')
  assertEquals(
