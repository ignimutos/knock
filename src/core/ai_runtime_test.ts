...
test('[contract] aiRuntime: summarize 显式 language 时直接生成目标语言摘要并应用 summaryLength', async () => {
  const calls: Array<Record<string, unknown>> = []
  const runtime = createAiRuntime({
    ai: createResolvedAiConfig(),
    defaultLanguage: 'zh-CN',
...
test('[contract] aiRuntime: summarize 未达 triggerLength 时直接返回原文且不调用 AI', async () => {
  const calls: Array<Record<string, unknown>> = []
  const runtime = createAiRuntime({
    ai: createResolvedAiConfig(),
    defaultLanguage: 'zh-CN',
  ...
  const out = await runtime.summarize(runtime.createEntryRuntime('s1', 'e1'), 'hello', {
    triggerLength: 6,
  })
...
test('[contract] aiRuntime: summarize 达到 triggerLength 时进入摘要路径', async () => {
  const calls: Array<Record<string, unknown>> = []
  const runtime = createAiRuntime({
    ai: createResolvedAiConfig(),
    defaultLanguage: 'zh-CN',
