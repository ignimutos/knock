...
test('[contract] sourceRuntimeHelpers: resolveSourceConfig 应返回命中的 source config', () => {
  const source = {
    id: 'rust',
    enabled: true,
    deliveries: [],
...
test('[contract] sourceRuntimeHelpers: resolveSourceConfig 缺失时应显式失败', () => {
  assertThrows(() => resolveSourceConfig({}, 'rust'), Error, 'source 未定义: rust')
})

test('[contract] sourceRuntimeHelpers: selectSourceInputGateway 应按 source kind/fetcher 选择 gateway', () => {
