...
test('[contract] production composition: runImmediate 应通过 RunDueSourcesUseCase 统一判定 due source', async () => {
...
test('[contract] production composition: 应暴露 queryRunsUseCase 并回读 run/item/attempt', async () => {
...
test('[contract] production composition: 应暴露 pruneFactsUseCase 并删除过期 run 与 dedupe', async () => {
  const factsDb = createInMemoryDb()
