import { getEnv, getEnvObject } from '../../src/platform/env.ts'
...
function shouldEnableImmediate(value: string | undefined = getEnv('KNOCK_IMMEDIATE')): boolean {
  switch (value) {
    case undefined:
    case '':
      return false
...
function resolveTargetMode(args: string[]): 'web' | 'daemon' | 'all' {
...
function normalizeAppArgs(rawArgs: string[]): string[] | undefined {
